    ...
    <div className="bg-[#202123] w-[260px] h-screen flex flex-col p-2 text-white hidden md:flex relative">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="flex flex-col gap-2">
          {chats
            .filter(chat => !chat.customModel) // Hide custom model chats from main list
            .length > 0 ? (
            chats
              .filter(chat => !chat.customModel)
              .map((chat) => (
                <div
                  key={chat.id}
          ...
          <div className="mt-6 border-t border-white/10 pt-4">
            ...
            <div className="flex flex-col gap-2">
              {customModels.map((model) => {
                // Find existing chat for this custom model
                const existingChat = chats.find(chat => chat.customModel?.id === model.id);
                  ...
                  <div
                    ...
                    onClick={() => {
                      if (onSelectCustomModel) {
                        // If chat exists, open it; otherwise create new
                        onSelectCustomModel(existingChat?.id || null, model);
                      } else {
                        onNewChat();
                      }
                    }}
